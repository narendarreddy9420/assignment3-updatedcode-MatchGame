...
import MatchGameImages from '../MatchGameImages'
...
class MatchGame extends Component {
  ...
  onClickImage1 = id => {
    ...
    const random = Math.ceil(Math.random() * length) - 1
    const randomImageId = imagesList[random].id
    if (id === imageId) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({imageId: randomImageId})
    }
  }
  ...
  render() {
    const timer1 = this.getTimer()
    const {score, timer, imageId} = this.state
    const image1 = imagesList.find(each => each.id === imageId)
    const image = image1.imageUrl
    ...
    return (
      <div>
        <div>
          <nav>
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-bg.png"
              alt=""
            />
            <div>
              <img
          ...
          <ul>
            {tabsList.map(each => (
              <MatchGameTabs
                eachTab={each}
                key={each.tabId}
                onChangeTab1={this.onChangeTab1}
              />
            ))}
          </ul>
          <ul>
            {filteredList.map(each => (
              <MatchGameImages
                eachImage={each}
                key={each.id}
                onClickImage1={this.onClickImage1}
              />
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <p>Your Score</p>
          <p>{score}</p>
          <button type="button" onClick={this.onRestart}>
            <img
